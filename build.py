...
def build_work(w, prev_w, next_w):
  ...
  <div class="wrap">
    <div class="work-images">
      <div class="zoom-wrap"><img id="work-main" class="work-main" src="{esc(w["image"])}?v=11" alt="{esc(w["title"])}, {esc(w["year"])}" width="1024" height="1024"></div>
      <div class="work-thumbs">
        <button class="thumb active" data-src="{esc(w["image"])}?v=11" aria-label="Artwork"><img src="{esc(w["image"])}?v=11" alt="" loading="lazy"></button>
        <button class="thumb" data-src="/assets/works/{esc(w["slug"])}-room.jpg?v=11" aria-label="In situ"><img src="/assets/works/{esc(w["slug"])}-room.jpg?v=11" alt="{esc(w["title"])} in situ" loading="lazy"></button>
        <button class="thumb" data-src="/assets/works/{esc(w["slug"])}-detail.jpg?v=11" aria-label="Detail"><img src="/assets/works/{esc(w["slug"])}-detail.jpg?v=11" alt="{esc(w["title"])} detail" loading="lazy"></button>
      </div>
      <script>(function(){{var m=document.getElementById('work-main'),t=document.querySelectorAll('.work-thumbs .thumb');t.forEach(function(b){{b.addEventListener('click',function(){{m.src=b.dataset.src;t.forEach(function(x){{x.classList.remove('active')}});b.classList.add('active')}})}});var zw=document.querySelector('.zoom-wrap');if(zw){{var zd=false,moved=false,touch=false;var lp=document.createElement('div');lp.className='zoom-loupe';zw.appendChild(lp);function setO(cx,cy){{var r=zw.getBoundingClientRect();var x=Math.min(Math.max(cx-r.left,0),r.width),y=Math.min(Math.max(cy-r.top,0),r.height);lp.style.left=x+'px';lp.style.top=y+'px';m.style.transformOrigin=(x/r.width*100)+'% '+(y/r.height*100)+'%';}}function tog(cx,cy){{zd=!zd;m.style.transform=zd?'scale(2.4)':'';zw.classList.toggle('zoomed',zd);lp.style.display=zd?'block':'none';if(zd&&cx!=null)setO(cx,cy);}}zw.addEventListener('click',function(e){{if(touch)return;tog(e.clientX,e.clientY);}});zw.addEventListener('mousemove',function(e){{if(zd)setO(e.clientX,e.clientY);}});zw.addEventListener('mouseenter',function(){{if(!touch&&zd)lp.style.display='block';}});zw.addEventListener('mouseleave',function(){{if(!zd)lp.style.display='none';}});zw.addEventListener('touchstart',function(e){{touch=true;moved=false;}},{{passive:true}});zw.addEventListener('touchmove',function(e){{if(!zd)return;moved=true;e.preventDefault();var p=e.touches[0];setO(p.clientX,p.clientY);}},{{passive:false}});zw.addEventListener('touchend',function(e){{if(moved)return;var p=e.changedTouches[0];tog(p.clientX,p.clientY);e.preventDefault();}},{{passive:false}});}}}})();</script>
